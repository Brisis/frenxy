          ...
          Positioned(
            ...
            child: GestureDetector(
              ...
              onVerticalDragEnd: (DragEndDetails details) {
                bool positiveDragThresholdMet = (_cardOffset < -_containerSize.height * widget.swipePositionThreshold || details.primaryVelocity! < -widget.swipeVelocityThreshold);

                bool negativeDragThresholdMet = (_cardOffset > _containerSize.height / widget.swipePositionThreshold || details.primaryVelocity! > widget.swipeVelocityThreshold);
                ...
                if (positiveDragThresholdMet && _cardIndex < widget.contentSize - 1) {
                  ...
                  _pendingEvent = ScrollEvent(ScrollDirection.FORWARD, ScrollSuccess.FAILED_END_OF_LIST, widget.contentSize - 1);
                  _state = DragState.animatingToCancel;
                } else {
                  // Thresholds not met so relaxing back to initial state
                  _pendingEvent = ScrollEvent(_cardOffset < 0 ? ScrollDirection.FORWARD : ScrollDirection.BACKWARDS, ScrollSuccess.FAILED_THRESHOLD_NOT_REACHED, null);
                  _state = DragState.animatingToCancel;
                }
                setState(() {
                  _dragState = _state;
